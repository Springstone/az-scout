...
- **Vendored all frontend CDN assets into the app bundle (no external CDN at runtime)** – Bootstrap, Bootstrap Icons (+ fonts), simple-datatables, highlight.js (+ both themes and the JSON language), marked, and D3 are now served from `src/az_scout/static/vendor/` via the existing `/static` mount instead of `cdn.jsdelivr.net` / `d3js.org`. The UI now works fully offline/air-gapped and identically across local dev, SaaS publishing (the files ship in the wheel), and customer self-hosting — with no per-mode configuration. As a result the Content-Security-Policy no longer allows external CDN hosts (`script-src`/`style-src`/`font-src`/`connect-src` are now `'self'` only). A new dependency-free maintenance script, `tools/vendor_assets.py`, (re)downloads the pinned versions when they are bumped (`python tools/vendor_assets.py`, `--check` to verify presence); the committed files remain the source of truth, so no build tooling is added to the normal dev/install/publish flows.
- **Plugins should now vendor their own third-party assets (recommended convention, not a breaking change)** – because the core CSP is `'self'`-only, plugins can no longer load third-party JS/CSS/fonts from an external CDN (such links are blocked at runtime). Plugin static dirs are served same-origin, so plugins should (1) reuse the libraries the core already vendors — Bootstrap, Bootstrap Icons, D3, marked, highlight.js, simple-datatables — and (2) vendor any *additional* dependency into their own `static/vendor/` and reference it via `/plugins/{name}/static/vendor/…`. Plugins that only reuse core libraries need no changes. The plugin authoring guidance (`.github/instructions/plugin-*.instructions.md`, `docs/plugins/index.md`, the `create-plugin` skill) and the plugin scaffold now document this; scaffolded plugins ship a `static/vendor/README.md` explaining the pattern.
